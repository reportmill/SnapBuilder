...
void dialogBoxClosed()
{
    ...
    //if(_selIndex==0 && _epane.getContent().getChildCount()==0) return;
    _epane.getEditor().setContent(getDoc(_selIndex));
...
private void buildUI()
{
    ...
    for(int i=0; i<_docNames.length; i++) { String name = _docNames[i];
        ...
        if(i==0) { label.setFill(Color.BLUE); label.setTextFill(Color.WHITE); }
